...
class ModelBasedCartPoleWrapper(ModelBasedWrapper):
    ...
    def cost_fn_tf_batch_v1(self, states, actions, next_states):
        x = next_states[:, 0]
        theta = next_states[:, 2]
    ...
    def cost_fn_tf_batch_v2(self, states, actions, next_states):
        x = next_states[:, 0]
        theta = next_states[:, 2]
        return tf.abs(x) / self.x_threshold + tf.abs(theta) / self.theta_threshold_radians

    def cost_fn_tf_batch(self, states, actions, next_states):
        if self.version == 'v1':
            return self.cost_fn_tf_batch_v1(states, actions, next_states)
        elif self.version == 'v2':
            return self.cost_fn_tf_batch_v2(states, actions, next_states)
        else:
            raise NotImplementedError
...
class ModelBasedRoboschoolReacher(ModelBasedWrapper):
    ...
    def cost_fn_tf_batch(self, states, actions, next_states):
        ...
        stuck_joint_cost = 0.1 * (tf.abs(tf.abs(gamma) - 1) < 0.01)
