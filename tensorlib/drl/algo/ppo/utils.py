...
class PPOReplayBuffer(ReplayBuffer):
    ...
    def random_iterator(self, batch_size):
        ...
        data_loader = tf.data.Dataset.from_tensor_slices((states, actions, reward_to_go, gaes, old_log_prob)).shuffle(
            states.shape[0]).batch(batch_size, drop_remainder=True)
