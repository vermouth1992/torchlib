...
class CriticNetwork(object):
    ...
    def action_gradients(self, inputs, actions):
        inputs = torch.from_numpy(inputs).type(FloatTensor)
        actions = torch.tensor(actions, requires_grad=True).type(FloatTensor)
...
class DDPG(deep_rl.BaseAgent):
    def __init__(self, actor, actor_optimizer, critic, critic_optimizer, tau):
        self.actor = ActorNetwork(actor, actor_optimizer, tau)
        self.critic = CriticNetwork(critic, critic_optimizer, tau)
    ...
    def load_checkpoint(self, checkpoint_path):
        self.actor.load_checkpoint(checkpoint_path)

    def save_checkpoint(self, checkpoint_path):
        self.actor.save_checkpoint(checkpoint_path)

    def train(self, env, actor_noise, total_timesteps, replay_buffer_type='normal', replay_buffer_config=None,
        ...
        for global_step in range(total_timesteps):
            ...
            if actor_noise == 'param_noise':
                ...
                if replay_buffer_type == 'frame':
                    ...
                    action = self.actor.predict(np.expand_dims(previous_observation, axis=0))[0]
            ...
            if global_step % log_every_n_steps == 0:
                ...
                if mean_episode_reward > best_mean_episode_reward:
                    if actor_checkpoint_path:
                        self.actor.save_checkpoint(actor_checkpoint_path)
                    if critic_checkpoint_path:
                        self.critic.save_checkpoint(critic_checkpoint_path)
